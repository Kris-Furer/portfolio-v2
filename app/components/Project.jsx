...
export default function Project({ id, title, subtitle, role, description, imgs, tools, liveLink }) {
  ...
  return (
    <div id={id} className="project my-8 text-white flex flex-col lg:flex-row gap-7  max-w-[2000px] m-auto ">
      ...
      <div className="sticky-container relative lg:w-[30%]">
        <div className="sticky flex gap-5 items-center top-0 left-0 lg:h-lvh">
          <div className="project-text pt-9">
            ...
            {/* Desktop Button ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */}
            {liveLink &&
              <a target="_blank" href={liveLink} className='rounded-full items-center justify-between text-center border border-white py-3 px-12 mt-6 hidden lg:inline-flex hover:bg-white hover:text-black transition ease-in-out duration-300'>
      ...
      {/* Mobile Button ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */}
      {liveLink &&
        <a target="_blank" href={liveLink} className='rounded-full lg:hidden items-center justify-between text-center border border-white py-3 px-12 mb-6 hover:bg-white hover:text-black transition ease-in-out duration-300'>
          See it Live <MdNavigateNext size="25" className='inline' />
        </a>
      }
    </div>
